...
export function DefaultErrorFallback({
  ...
  return (
    <div
      className={twMerge(
        "rounded-lg border border-red-300 bg-red-50 p-6 text-red-900 shadow-sm",
        className,
      )}
    >
      ...
      {error.stack && (
        <details
          className="mb-4 rounded border border-red-300 bg-red-100 p-3"
          open={false}
        >
          <summary className="cursor-pointer font-semibold">
            스택 트레이스
          </summary>
          <pre className="mt-2 overflow-x-auto text-sm whitespace-pre-wrap text-red-800">
            {error.stack}
          </pre>
        </details>
      )}
